...
The Teams gateway turns a connected, licensed Microsoft 365 user into a 1:1 Teams agent backed by
Claude Code or OpenAI Codex. It runs inside OpenConnector: Paperclip, project mappings, and a second
credential store are not required.
...
1. Configure the Microsoft Teams OAuth client, then connect each Microsoft 365 account that should
   ...
   `Chat.Read`, `Chat.Create`, and `ChatMessage.Send`.
...
4. Enter at least one internal email domain and choose the exact OpenConnector connections that the
   agent may use.
5. Save the agent. The Node/Docker runtime polls Teams every 30 seconds by default. Set
...
Threads are durable, isolated per Teams agent and chat, and processed with bounded concurrency. The
configured thread window controls when idle conversation context expires.
...
Gateway agent configuration, contacts, cursors, conversations, pending plans, and pending approval
links are stored in the runtime database and pass through the configured secret codec.
