...
import type { FeedApprovalSummary, FeedComment, FeedItem, FeedPage, FeedThread, IFeedStore } from "./feed-types.ts";
...
function summarizeTrigger(run: FlowRun): {
  ...
  return {
    title: subject ?? `${humanize(run.trigger)} triggered ${run.flowSnapshot.name}`,
    summary: summary
